...
def plotTemporalEvolutionPrice(t,V,Pmin,Pmax,xlabel,ylabel,color_to_plot1,filename):
  q = np.arange(0,101,step=10)
  IC_V =  np.percentile(V,q,axis=1)
  fig = plt.figure(figsize=(8,4))
  ax = fig.add_subplot(1, 1, 1)
  ax.plot(t, IC_V[5,:], label='Median', color=color_to_plot1, alpha=0.9)
  for k in range(5):
    ax.fill_between(t, IC_V[k,:], IC_V[10-k,:], color=color_to_plot1, alpha=0.1)
  ax.plot(t,Pmin, label='P_min', color='blue', linestyle='--', marker='o' ,markevery=3*365)
  ax.plot(t,Pmax,label='P_max', color='red', linestyle='--', marker='x',markevery=3*365)
...
def histogramTotalPopulationOverTime(t,data,filename):
  Xtotal = data[:,:,0]+data[:,:,1]
  K = Xtotal.shape[0]
  minX = np.min(Xtotal)
  maxX = np.max(Xtotal)
  nbins = 200
  ...
  ax.imshow(h2d, extent=[t[0] , t[-1],maxX, minX], aspect='auto')#, norm=LogNorm(), cmap='plasma')
  pos  = ax.invert_yaxis()
...
def histogramOverTime(t,V,ylabel,filename):
  Xtotal=V
  K = Xtotal.shape[0]
  minX = np.min(Xtotal)
  maxX = np.max(Xtotal)
...
def plotRateFunctions(eta,Pbase,Sub,parR,parFmaxA,parFmaxJ,dir):
  E = np.linspace(0,1,50)
  P = 10 
  R=rJ(E,P*np.ones_like(E),'harvesting','power-law',eta,0,0,parR)
  FA=FmaxA(E,P*np.ones_like(E),'harvesting','power-law',eta,0,0,parFmaxA)
  FJ=Fmaxj(E,P*np.ones_like(E),'harvesting','power-law',eta,0,0,parFmaxJ)
  plt.plot(E,R)
  filename = dir+'/rJ.png'
  plt.savefig(filename,dpi=400)
  plt.close()
  plt.plot(E,FA)
  filename = dir+'/FmaxA.png'
  plt.savefig(filename,dpi=400)
  plt.close()
  plt.plot(E,FJ)
  filename = dir+'/FmaxJ.png'
  plt.savefig(filename,dpi=400)
  plt.close()
  return
...
'''----------------------------------------------------'''
'''----------------------------------------------------'''
""".   ## Funciones para la dinámica poblacional.      """
'''----------------------------------------------------'''
'''----------------------------------------------------'''
'''----------------------------------------------------'''
'''----------------------------------------------------------------------------------------------------------------------------------------'''
...
def main():
  ...
  plotTemporalEvolutionPrice(t[N1:],P,PminH,PmaxH,'Time [years]','Price','black',dirName+"/price.png")
  plotRateFunctions(eta,Pbase,Sub,params_rj,params_FmaxA,params_FmaxJ,dirName)
  histogramTotalPopulationOverTime(t[0:N1+2],XNH,dirName+"/histogramsOverTimeTotalPopulationNonHarvesting.png")
  histogramTotalPopulationOverTime(t[N1+1:],X,dirName+"/histogramsOverTimeTotalPopulationDynamicModel.png")
  histogramTotalPopulationOverTime(t[N1+1:],X_n,dirName+"/histogramsOverTimeTotalPopulationFullNonCompliance.png")
  histogramTotalPopulationOverTime(t[N1+1:],X_c,dirName+"/histogramsOverTimeTotalPopulationFullCompliance.png")
  histogramOverTime(t[N1:],E,'E',dirName+"/hist_E.png")
  histogramOverTime(t[N1:],X[:,:,0],'Biomass [g/m2]',dirName+"/hist_XA.png")
  histogramOverTime(t[N1:],X_c[:,:,0],'Biomass [g/m2]',dirName+"/hist_XA_c.png")
  histogramOverTime(t[N1:],X_n[:,:,0],'Biomass [g/m2]',dirName+"/hist_XA_n.png")
  histogramOverTime(t[N1:],X[:,:,1],'Biomass [g/m2]',dirName+"/hist_XJ.png")
  histogramOverTime(t[0:N1+2],XNH[:,:,0],'Biomass [g/m2]',dirName+"/hist_XA_NH.png")
  histogramOverTime(t[0:N1+2],XNH[:,:,1],'Biomass [g/m2]',dirName+"/hist_XJ_NH.png")
  histogramOverTime(t[N1:],X_c[:,:,1],'Biomass [g/m2]',dirName+"/hist_XJ_c.png")
  histogramOverTime(t[N1:],X_n[:,:,1],'Biomass [g/m2]',dirName+"/hist_XJ_n.png")
  histogramOverTime(t[N1:],TasaNacH,'rJ',dirName+"/hist_rJ.png") 
  histogramOverTime(t[N1:],TasaExtracion[:,:,0],'Extraction Rate Adults Dynamic',dirName+"/hist_ExtA.png")
  histogramOverTime(t[N1:],TasaExtracion[:,:,1],'Extraction Rate Juvenile Dynamic',dirName+"/hist_ExtJ.png") 
  ...
  ################################### Plots by year
  ##############################################################################################################################################
  for k in range(1,yearsHarvesting+1):
